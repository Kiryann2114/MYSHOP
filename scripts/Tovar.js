function LoadTov(i){
    ...
    document.getElementsByClassName('name_tov').item(0).textContent = database[i].name;
    document.getElementsByClassName('op_tov').item(0).textContent = database[i].opisanie;
    document.getElementsByClassName('zena').item(0).textContent = database[i].zena.toString()+"₽";
    document.getElementsByClassName('kolich').item(0).textContent = database[i].kol;
    document.getElementsByClassName('im_tov').item(0).src = database[i].images[imActiv];
    if(!database[i].inkorz){
        document.getElementsByClassName('btn_buy').item(0).textContent = "Добавить в корзину";
        document.getElementsByClassName('kol').item(0).style = "display:block"
    }
    else{
        document.getElementsByClassName('btn_buy').item(0).textContent = "Перейти в корзину";
        document.getElementsByClassName('kol').item(0).style = "display:none"
    }
    document.getElementsByClassName('btn_buy').item(0).onclick = function (){
        if(!database[i].inkorz){
